...
def start(core: VACore):
    manifest = {
        ...
        "commands": {
            "поболтаем|поговорим|давай поболтает|давай поговорим|мне грустно": run_start,
            "справка": run_start_spravka,
        },

        "tts": {
            "vsegpt": (init, None, towavfile)
        }
    }
    return manifest
...
def run_start(core: VACore, phrase: str):
    ...
    if phrase == "":
        core.play_voice_assistant_speech("Давай поговорим!")
...
def new_chat(core: VACore):
    options = core.plugin_options(modname)
    core.chatapp = ChatApp(model=options["model"], system=options["system"])


def new_chat_spravka(core: VACore):
    options = core.plugin_options(modname)
    core.chatapp = ChatApp(model=options["model_spravka"])


def boltalka(core: VACore, phrase: str):
    if phrase == "отмена" or phrase == "пока" or phrase == "Ирина отмена":
        core.play_voice_assistant_speech("Пока!")
        return

    if phrase == "новый диалог" or phrase == "новые диалог":
        new_chat(core)
        core.play_voice_assistant_speech("Давай поговорим о чем-нибудь другом")
        core.context_set(boltalka, 20)
        return

    try:
        response = core.chatapp.chat(phrase)
...
def boltalka_spravka(core: VACore, phrase: str):
    ...
    try:
        ...
        print('Запрос:',prompt)
        response = core.chatapp.chat(prompt)
        core.say(response["content"])
