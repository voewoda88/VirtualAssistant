...
def parseTextAtWidth(text, widthWidget1, widthWidget2, indentX=0):
    widthText = widthWidget2 - indentX * 2
...
class App(tk.Tk):
    settings = {
        ...
        'fontTitle': ('default', 10, 'bold'),
        # шрифт текста загловка, ('Шрифт', размер, 'normal bold  italic underline')
        ...
        'fontCloseButton': ('default', 10, 'bold'),
        # шрифт текста кнопки закрытия, ('Шрифт', размер, 'normal bold  italic underline')
        ...
        'fontMinimizeButton': ('default', 10, 'bold'),
        # шрифт текста кнопки сворачивания, ('Шрифт', размер, 'normal bold  italic underline')
        ...
        'bgMessageBox': '#18191d',
        # цвет бокса сообщения, он размером со ширину окна и обычного того же цвета, что фрейм
        ...
        'isShowSayAssistantName': True,
        # открывает окно после произношения "Ирина" (или любого другого Имени ассистента)
        'isShowCommand': True,
        # показывать окно после произношения обычных команд 'Ирина привет' (или любого другого Имени ассистента)
        'isSoundStart': False,  # прогигрывает звук после произношения "Ирина" (или любого другого Имени ассистента)
        'pathSoundStart': os.path.join('media', 'micro_up.wav'),
        # путь к звуку прогнориуему после произношения "Ирина" (или любого другого Имени ассистента)
        'isSoundEnd': False,
        # прогигрывает звук после отсуствия ответа произношения "Ирина" (или любого другого Имени ассистента)
        'pathSoundEnd': os.path.join('media', 'micro_down.wav')
        # путь к звуку прогнориуему после отсуствия ответа произношения "Ирина" (или любого другого Имени ассистента)
    ...
    def main(self):
        ...
        while True:
            ...
            if self.rec.AcceptWaveform(data):
                ...
                # скрывает приложение
                if self.time_start_assistant != -1 and time.time() - self.time_start_assistant > self.settings[
                    'timeWaiting']:
...
class MessageList:
    def __init__(self, root, maxCountMessage, maxLengthMessage, indentXMessegeBox, indentYMessegeBox,
                 indentXTextMessege, indentYTextMessege, bgMessageBox, bgRight, fgRight, bgLeft, fgLeft, font_message):
        self.messageList = []
        self.root = root
